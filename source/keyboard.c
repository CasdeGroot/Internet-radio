...
/*----------- user added functions ----------------------------------------- */
 /*
	NAME: getkeyfound
	DESCRIPTION: retu the KeyFound variable
	RETURNS: u_short value of KeyFound.
 */

u_short get_keyfound() {

   return KeyFound;
}

THREAD(fall_asleep_thread, arg)
{
	long sleep_timer = 6000; //5 min
	long backlight_timer = 400; //20 seconden
	long times_reported = 1;
	//NutThreadSetPriority(30);
	for (;;) {
	   ...
	   if(times_reported > sleep_timer)
	   {
		  go_home(1);
	   }
	   
	   if(times_reported > sleep_timer +2)
	   {
		   times_reported = 1;
		   go_home(0);
	   }
	   
	   if((times_reported > backlight_timer) && (times_reported < (backlight_timer+2)))
	   {
		 LcdBackLight(LCD_BACKLIGHT_OFF);
	   }
		
		if(!(times_reported %100))
		{
			// LogMsg_P(LOG_INFO,PSTR("times: %d"),times_reported);
		}
       times_reported++;	
	   NutSleep(50);
   }
...
// /*
// * NAME: go_home
// * DESCRIPTION: calls home_state to go to the idle screen
// * PARAMS: None
// * RETURNS: void
// */
void go_home(int i)
...
// /*
// * NAME: start_checking_input
// * DESCRIPTION: method to create a thread, which checks if an input has been given
// * PARAMS: None
// * RETURNS: void
// */
void start_checking_input(void)
{
	LogMsg_P(LOG_INFO,PSTR("CALLED input check"));
	NutThreadCreate("asleep thread", fall_asleep_thread, 0,512);
}
